...
class Tooltip {
    ...
    _applyPosition (coordinate : Coordinate) : (class_names : Array<string>|string) => void {
        this.tooltip.style.top  = coordinate.top
        this.tooltip.style.left = coordinate.left

        return (class_names)  => {
            const normalized = Array.isArray(class_names) ? class_names.join(" ") : class_names
            if(normalized) {
                this.tooltip.classList.add(normalized)
            }
        }
    }
    /**
    ...
    autoPlace (left_cushion : number = 0, top_cushion : number = 0){
        ...
        const coordinate      = {...left_coordinate, ...top_coordinate}
